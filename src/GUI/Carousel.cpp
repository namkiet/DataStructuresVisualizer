...
void Carousel::pushItem(Item::ItemType type, std::string imageName, GUI::Button::Callback callback)
{
    ...
    {
        items[i]->mInitialPosition = sf::Vector2f(WINDOW::ScreenWidth / 2 + i * 300, WINDOW::ScreenHeight / 2);
        items[i]->setDotPosition(sf::Vector2f(WINDOW::ScreenWidth / 2 + i * 20 - 20, 600));
    }
}
...
{
    if (abs(targetOffset - currentOffset) < 1e-3)
